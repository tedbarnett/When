/**
 * POST /api/calendars/teds-dublin/import — owner-only paste-a-URL event importer.
 *
...
/** Rebuild a Date as 'YYYY-MM-DDTHH:MM:SS±HH:MM' in Europe/Dublin. */
function toNyIso(d) {
...
/**
 * Normalize a startDate-ish string to ISO8601 with the Dublin offset.
 * Accepts full ISO (any offset / Z — converted to Dublin wall time), naive
 * datetimes (assumed Dublin local), and bare dates (midnight, time TBD).
...
export function normalizeStart(raw) {
  ...
  if (m) {
    // naive: assume Dublin local; offset approximated from the date itself
...
export function jsonldToRaw(ld) {
  ...
  if (typeof loc === 'string') venue = loc;
    ...
    if (/dublin|baile atha cliath/i.test(venue) && venue === neighborhood) neighborhood = '';
  }
  return {
...
async function aiExtract(env, pageText, pageUrl) {
  ...
  const prompt =
    ...
    ' (Dublin, Ireland). Page URL: ' + pageUrl + '\n\n' +
    'Answer with STRICT JSON only — no prose, no markdown fences. Shape:\n' +
    '{"title":"","venue":"","neighborhood":"","start":"ISO8601 with +00:00 or +01:00 Dublin offset","end":"optional, same format","price":"e.g. Free, €25, €20–40","url":"the event page URL","blurb":"<=140 chars, strictly factual, from the page"}\n\n' +
