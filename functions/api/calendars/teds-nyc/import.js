/**
 * POST /api/calendars/teds-nyc/import — owner-only paste-a-URL event importer.
 *
...
/** Rebuild a Date as 'YYYY-MM-DDTHH:MM:SS±HH:MM' in America/New_York. */
function toNyIso(d) {
...
/**
 * Normalize a startDate-ish string to ISO8601 with the NY offset.
 * Accepts full ISO (any offset / Z — converted to NY wall time), naive
 * datetimes (assumed NY local), and bare dates (midnight, time TBD).
...
export function normalizeStart(raw) {
  ...
  if (m) {
    // naive: assume NY local; offset approximated from the date itself
...
export function jsonldToRaw(ld) {
  ...
  if (typeof loc === 'string') venue = loc;
    ...
    if (/new york|nyc|brooklyn|queens|manhattan|bronx/i.test(venue) && venue === neighborhood) neighborhood = '';
  }
  return {
...
async function aiExtract(env, pageText, pageUrl) {
  ...
  const prompt =
    ...
    ' (New York). Page URL: ' + pageUrl + '\n\n' +
    'Answer with STRICT JSON only — no prose, no markdown fences. Shape:\n' +
    '{"title":"","venue":"","neighborhood":"","start":"ISO8601 with -04:00 or -05:00 New York offset","end":"optional, same format","price":"e.g. Free, $25, $20–40","url":"the event page URL","blurb":"<=140 chars, strictly factual, from the page"}\n\n' +
